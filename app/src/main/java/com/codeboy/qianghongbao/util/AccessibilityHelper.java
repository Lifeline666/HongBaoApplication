...
private static final String TAG=AccessibilityHelper.class.getSimpleName();
    ...
    public static AccessibilityNodeInfo findNodeInfosById(AccessibilityNodeInfo nodeInfo, String resId) {
        Log.i(TAG, "SDK初始化值" +Build.VERSION.SDK_INT);
        Log.i(TAG, "果冻jelly值" +Build.VERSION_CODES.JELLY_BEAN_MR2);
   ...
   // @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    ...
    public static void performClick(AccessibilityNodeInfo nodeInfo) {
        Log.i(TAG, "工具类开始点击事件，进来对象为>>>>>>>>>>>>>"+nodeInfo);
        if(nodeInfo == null) {
            Log.i(TAG, "进来的入参为null了>>>>>>>>>>>>>");
            return;
        }
        Log.i(TAG, "控件是否被点击了>>>>"+nodeInfo.isClickable());
